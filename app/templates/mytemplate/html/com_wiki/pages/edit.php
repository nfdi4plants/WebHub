...
<section class="main section">
	...
	<?php } else {*/ ?>
			...
			<?php } ?>
					...
					<label for="page_access">
						<?php echo "Page Access" ?>: <span class="required"><?php echo Lang::txt('COM_WIKI_REQUIRED'); ?></span>
						<select name=page[access]" id="page_access" class="form-control">	
						<?php foreach(Viewlevel::all()->rows() as $level) { ?>
							<option value="<?php echo $level->get('id'); ?>"<?php if ($this->page->get('access') == $level->get('id')) { echo ' selected="selected"'; } ?>><?php echo $level->get("title"); ?></option>
						<?php } ?>
						</select>
					</label>
                                    </div>
